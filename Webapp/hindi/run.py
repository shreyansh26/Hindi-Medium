...
def main(link, user):
	...
	f_en = open("english_subtitles_" + str(user)+".srt", "w")
	f_hi = open("hindi_subtitles_" + str(user)+".srt", "w")

	num_files = len(os.listdir('./Splits_' +str(user)))
	cnt = 0
	start = 0
	end = 5
	...
	f_en.close()
	f_hi.close()
	files = glob.glob('./Splits_'+str(user)+'/*')
	for f in files:
	    os.remove(f)
